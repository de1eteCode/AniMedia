...
public class RegistrationCommandHandler : IRequestHandler<RegistrationCommand, Result<AuthorizationResponse>> {
    ...
    public RegistrationCommandHandler(IApplicationDbContext context, ITokenService tokenService,
        IHashService hashService, IOptions<JwtSettings> jwtSettings) {
        _context = context;
    ...
    public async Task<Result<AuthorizationResponse>> Handle(RegistrationCommand request,
        ...
        var isUserByNicknameExists =
            await _context.Users.AnyAsync(e => e.Nickname.Equals(request.Nickname), cancellationToken);
        ...
        return new Result<AuthorizationResponse>(new AuthorizationResponse(newUser.UID, accessToken,
